...
extrap_upper = np.array([-2 * slope_upper +
                         sum(np.array([1, 2, 4, 8, 16]) * coeffs[-1]),
                         slope_upper, 0, 0, 0])

slope_lower = sum(np.array([0, 1, -2, 3, -4]) * coeffs[0])
extrap_lower = np.array([slope_lower +
                         sum(np.array([1, -1, 1, -1, 1]) * coeffs[0]),
                         slope_lower, 0, 0, 0])
...
class RadiativeCoolingRate(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        X_vector_all = np.array([np.ones_like(X_all), X_all,
                                 X_all**2, X_all**3, X_all**4])

        in_range = []
        for i in range(len(new_X_bounds)-1):
            in_range.append(np.where(np.logical_and(X_all > new_X_bounds[i],
...
#         outputs["Lz"] = Lz
    ...
    def compute_partials(self, inputs, J):
        ...
        X_derv_vector_all = np.array([np.zeros_like(X_all), 10**(-X_all),
                                      2 * X_all * 10**(-X_all),
                                      3 * X_all**2 * 10**(-X_all),
                                      4 * X_all**3 * 10**(-X_all)])
        ...
        dPdt = (2 * sum(integrand_dT) - integrand_dT[0] - integrand_dT[-1]) * dρ / 2
